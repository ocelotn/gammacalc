...
//blind, dazed, prone (in melee), helpless, restrained, stunned, surprised grant combat advantage
	...
	if (oprone){
		if (wtype=="melee"){combatadvantage = true;}
		if ((wtype=="ranged" || wtype=="gun") && adjacent == false ){disavantage +=2;}
	}
...
function setup(){
	...
	//part B	
	var base = 'http://hills.ccsf.edu/~lorti001/cnit133/';

/*
	$('[name="b1"]').change(
		function(){
			window.location.href = base+'hw'+$('[name="b1"]').children(':selected').val()+".html";
		}
	);
	...
	$(go).click(
	function(){
		window.location.href = base+'HW5-'+$('[name="b2"]').children(':selected').val()+".html";
	});
	$(go).insertAfter($('[name="b2"]'));
*/
}
